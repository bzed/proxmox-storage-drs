...
|---|---|---|
| reserve / capacity constraint | §5.3 (C4), (C5), §5.3.1 | `soft_s` floor vs `f·Z` — the `max()`, both ways round; already-violating start; `r_s > 0` reporting; per-storage resolution (absolute, byte-unit string, `"N%"` against *that* storage's capacity); the written values validated |
