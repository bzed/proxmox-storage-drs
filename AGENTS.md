...
- The worked example in `IMPLEMENTATION_PLAN.md` §14 is an executable acceptance fixture:
  ...
  generated by exhaustive enumeration). **Never hand-edit the expected file** — change the
  input or the generator and re-run `python3 tests/fixtures/generate_fc_tier1.py`.
  `--check` asserts it is current and runs in CI.
