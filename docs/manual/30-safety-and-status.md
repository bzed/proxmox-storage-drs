...
- **The snapshot reserve is never traded for balance, and neither is the
  free space you configure.** `pve-storage-drs` will leave a group
  imbalanced rather than breach `snapshot_reserve.factor`/`free_space.soft`
  on any storage, and that reserve holds *during* a migration, not merely
  before and after it — down to `free_space.hard`, never below it.
...
| `verify-storages` | Implemented: `saferemove` and the implied wipe time per storage, the resolved `free_space.soft`/`.hard` bytes per storage (section 5.3.1), warning when `gates.cooldown_per_storage` or `migration.max_single_move_duration` is shorter than the implied wipe time. |
| `plan` | Implemented: per group, evaluates the section 6 gate (reading real drift history from `state.json` when a group has one recorded — same as `show-load`, see `docs/internals/15-state.md`), and on `ACT` solves it with `solver.backend` (`auto` tries CBC, then the dependency-free heuristic — see `docs/internals/91-optimize.md`; the MILP backend and the heuristic exclude, as a migration target, any storage within `gates.cooldown_per_storage`, except when repairing a live reserve violation, which is never deferred for a cooldown, in the heuristic only — see `docs/internals/91-optimize.md`'s own note on that gap), orders its moves under the section 8 transient reserve invariant, and checks the whole plan against section 7's payback (cost/benefit) test, including the section 7.3 saturation-ceiling guard (mirroring phase only) for any storage that configures `saturation_load` (still none in this project's own dogfooding cluster) — a plan whose executed endpoint reduces the group's reserve/free-space shortfall (section 5.3.1's `free_space.soft`) is exempt from the economic half of that test but not from the hard per-move duration rule or the saturation guard. Human and `--json` output report which backend actually solved each group. **No automatic re-solve-and-shrink on a failing payback test** — a plan that fails is reported, not silently adjusted (section 7.3's 3-retry loop is not implemented). See `docs/manual/27-plan.md` and `docs/internals/90-heuristic.md`/`91-optimize.md`/`95-schedule.md`/`96-payback.md`. |
| `explain` | Implemented: runs the identical per-group gate/solve/schedule/payback pipeline `plan` does (`_plan_group()`), then narrates what `plan`'s own output does not print. Lists every disk pinned this run with its exact reason (section 3.6/3.7 — snapshots, an exclusion, a lock, or a still-running `gates.cooldown_per_disk`), names any VM a pin leaves with disks spread across more than one storage ("cannot fully consolidate"), and breaks the section 5.4 objective down into its six terms (`ObjectiveBreakdown`'s own reason for keeping them separate rather than only a `total`) when the gate acted. Also reports pinned load as a fraction of the group's total against `report.warn_pinned_load_fraction`, flagging when the residual imbalance is likely structural (too much load pinned to fix by moving anything) rather than a planning shortfall — alongside the best achievable spread the plan actually reached given those pins. Human and `--json` output, same shape as `plan`'s with these fields added. See `docs/internals/40-cli-and-logging.md` and `docs/internals/90-heuristic.md`. |
...
| `collect-testdata`, and the global `--replay` | Implemented (`IMPLEMENTATION_PLAN.md` section 16, phase 10). `collect-testdata` captures everything the engine can read from the PVE API and Prometheus — the superset of every forecaster's data range, not just the configured one — replaces every identifier with a pseudonym keyed to a salt that never leaves the host (`support.salt_path`), and writes a bundle (directory + deterministic `.tar.gz`) you can read before deciding whether to send it to the author as test data; `--estimate` prints the query/sample-point estimate and refuses before fetching above `support.max_series_points`. `--replay BUNDLE` then runs `verify-metrics`, `verify-storages`, `show-load`, `plan` and `explain` against such a bundle with no network access at all, using the bundle's own `config.yaml` unless `-c` overrides it; `apply` and any `--mode` above `dry-run` are a usage error under `--replay` (exit `2`), and so is `collect-testdata` itself, which needs a live cluster. See `docs/manual/26-collect-testdata-and-replay.md` and `tests/corpus/README.md`, which documents what a bundle does and does not hide and the terms on which one is accepted into the project. |
...
and `jsonschema` installed. One dependency is still optional and is
imported only where it is used, never at module level:
...
and `solver.backend: auto` (the default) uses it. The dependency-free
heuristic of `IMPLEMENTATION_PLAN.md` section
...
what a plain `apt install pve-storage-drs` lands on by default. (An
optional `ortools`-based CP-SAT backend existed through release 0.1.8; it
was removed — `ortools` has no Debian package, so that backend could
never run on a PVE host. See `docs/internals/91-optimize.md`.)
