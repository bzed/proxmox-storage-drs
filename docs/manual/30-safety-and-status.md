...
`IMPLEMENTATION_PLAN.md` section 12 lays out ten phases. Phases 1–9 — and
with them every command in the table below except the last row — are
implemented; phase 10 (`collect-testdata`, `--replay` and the real-cluster
test corpus of section 16) is specified and not yet built. Dogfooding
against a production cluster continues, and `REVIEW.md` tracks findings
from that to resolution — being honest about exactly where a still-evolving
tool stands matters more than a document that reads as finished and closed:
...
| `collect-testdata`, and the global `--replay` | **Specified, not implemented** (`IMPLEMENTATION_PLAN.md` section 16, phase 10). `collect-testdata` will capture everything the engine can read from the PVE API and Prometheus — the superset of every forecaster's data range, not just the configured one — replace every identifier with a pseudonym keyed to a salt that never leaves the host, and write a bundle you can read before deciding whether to send it to the author as test data. `--replay BUNDLE` will then run `verify-metrics`, `verify-storages`, `show-load`, `plan` and `explain` against such a bundle with no network access at all (`apply` refuses). Until it lands neither exists on the command line at all: this build rejects both as a usage error, exit `2`. `tests/corpus/README.md` documents what a bundle does and does not hide, and the terms on which one is accepted into the project. |
